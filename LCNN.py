...
z = np.array(im)
z = batch_normalize(z)                 # normalize each pixel of input as (val - mean)/standard deviation
...
print (poolLayer.shape)

#initializing the transformation matrix and label information for the Label Consistent Module
size_transform_matrix = poolLayer.shape[0]*poolLayer.shape[1]*poolLayer.shape[2]
print (size_transform_matrix)
transform_matrix = np.random.rand(size_transform_matrix,size_transform_matrix)
label_info = np.random.randint(2,size=(size_transform_matrix,m))
...
(delta_weights,deltaW,deltabias,delta_transform_matrix) = BackPropagation(network,Y,predicted,z,Weights,biases,kernellist,biaseslist,poolLayer,convLayer,transform_matrix,alpha,label_info[:,0])
print (delta_weights)
print ("Hello\n\n\n\n")
print (deltaW)
print ("Hello\n\n\n\n\n")
...
##    kernellist.append(kernels[i])
##    biaseslist.append(Biases[i])
