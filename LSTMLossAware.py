...
class Elman:
	...
	def build_model(self):
		...
		self.rnn_cell = tf.contrib.rnn.BasicLSTMCell(self.hdim)
		output_states = []
		for i in range(self.batch_size):
			input_1 = tf.split(tf.reshape(self.input_[i],[1,self.indim]),self.indim,1)
			outputs,states = tf.contrib.rnn.static_rnn(self.rnn_cell,input_1,dtype=tf.float32)
			outputs2 =tf.matmul(tf.concat([outputs[-1],tf.constant(1,dtype=tf.float32,shape=[1,1])],1), (self.weights_hidden_output_binary * self.alpha3))			
			output_states.append(outputs2)
	...
	def train(self):
		...
		self.original_weights = tf.trainable_variables()
		for i in self.original_weights:
			print(i.name)
