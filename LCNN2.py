...
no_classes=10
learning_rate = 0.001
alpha = 5                     # used to control tradeoff between classification error and discriminative representation error
m = 100                       #no of training examples
no_epochs = 1                 # no of iterations for stochastic gradient descent

def initialize_kernel_bias(no_filters,filter_size):
	kernellist=list()
	biaseslist=list()
	...
	return(kernellist,biaseslist)

def initialize_weights_bias(output_size,input_size):
	mu =np.array([0 for i in range(output_size)])
	cov2 = np.array([1 for i in range(output_size)])
	cov = np.diag(cov2)
	Weights = np.random.multivariate_normal(mu,cov,input_size)
...
class ConvolutionalNN(object):
    ...
    def ConvolutionalLayer(self,X,kernellist,biaseslist,filter_size=(5,5,3),stride_length=(1,1),no_filters=3,padding=0):
        inputX = np.array(X)
        #padding = (filter_size[0]-1)/2
        
        W2 = int((X.shape[0] - filter_size[0] + 2*padding)/stride_length[0]) + 1
        H2 = int((X.shape[1] - filter_size[1] + 2*padding)/stride_length[1]) + 1
    ...
    def Label_Consistent_Transformation(self,X,transform_matrix,label_info):
        transformed_neurons = np.zeros((X.shape[0]*X.shape[1]*X.shape[2]))
        #z=0
        #for i in range(X.shape[0]):
        #    for j in range(X.shape[1]):
        #        for k in range(X.shape[2]):
        #            transformed_neurons[z] = X[i][j][k]
        #            z = z+1
        transformed_neurons = X.reshape(transformed_neurons.shape)
        mat = np.zeros((transformed_neurons.shape[0]))
        for i in range(transform_matrix.shape[0]):
            mat[i] = 0
            for j in range(transform_matrix.shape[1]):
                mat[i] += (transform_matrix[i][j]*transformed_neurons[j])
        difference = label_info - mat
        difference = difference**2
        total_sum = sum(difference)
        loss_r = math.sqrt(total_sum)
        return loss_r
...
#Here alpha is the hyperparamter for label consistency layer
def BackPropagation(network,yactual,ypredicted,z,Weights,biases,kernellist,biaseslist,poolLayer,convLayer,transform_matrix,alpha,label_info):
    ...
    for i in range(delta3.shape[0]):
        for j in range(delta3.shape[1]):
            for k in range(delta3.shape[2]):
                ...
                delta3[i][j][k] =sum
    delta_Lr = np.zeros((transform_matrix.shape[1]))
    #flattening the pooling layer transform_matrix is of size N_{l}*N_{l}
    transformed_neurons = np.zeros((poolLayer.shape[0]*poolLayer.shape[1]*poolLayer.shape[2]))
    transformed_neurons = poolLayer.reshape(transformed_neurons.shape)
    # mat represents the matrix transform_matrix * flattened layer for transformation
    mat = np.zeros((transformed_neurons.shape[0]))
    for i in range(transform_matrix.shape[0]):
        mat[i] = 0
        for j in range(transform_matrix.shape[1]):
            mat[i] += (transform_matrix[i][j]*transformed_neurons[j])
    #learned transformed_representation - q_{l}(ideal transformation representation)
    difference = mat - label_info
    for i in range(transform_matrix.shape[1]):
        delta_Lr[i]=0
        for j in range(transform_matrix.shape[0]):
            delta_Lr[i] += (difference[j]*transform_matrix[j][i])
    delta_Lr = delta_Lr*2*alpha
    delta_Lr = delta_Lr.reshape(poolLayer.shape)
    #delta_L w.r.t. x(i) for the layer with label consistency module
    delta_L = delta3+delta_Lr
    
    #delta of loss function w.r.t. the transformation matrix
    delta_transform_matrix = np.zeros(transform_matrix.shape)
    for i in range(difference.shape[0]):
        for j in range(transformed_neurons.shape[0]):
            delta_transform_matrix[i][j] = difference[i]*transformed_neurons[j]
    delta_transform_matrix = 2*alpha*delta_transform_matrix
    ...
    #Here delta_biases should be found for this FC layer
    ...
    return (delta_weights,deltaW,deltabias,delta_transform_matrix)

def gradient_descent(Weights,Biases,delta_weights,kernellist,biaseslist,deltaW,deltabias,learning_rate,transform_matrix,delta_transform_matrix):
    ...
    transform_matrix = transform_matrix - (learning_rate*delta_transform_matrix)
    return (kernellist,biaseslist,Weights,Biases,transform_matrix)    
    
        # normalize each pixel of input as (val - mean)/standard deviation
network = ConvolutionalNN()
(kernellist,biaseslist) = initialize_kernel_bias(no_filters,filter_size)
pool_layer_size=(48,48,3)
(Weights,biases) = initialize_weights_bias(no_classes,pool_layer_size)     # this size is obtained using the formula convLayer.size/(pool_size)
#initializing the transformation matrix and label information for the Label Consistent Module
size_transform_matrix = pool_layer_size[0]*pool_layer_size[1]*pool_layer_size[2]
#transform_matrix = np.random.rand(size_transform_matrix,size_transform_matrix)
transform_matrix = np.random.normal(0,0.1,(size_transform_matrix,size_transform_matrix))
label_info = np.random.randint(2,size=(m,size_transform_matrix))

train_data = np.zeros((m,100,100,3))
Y = np.zeros((m,no_classes))
#(train_data,Y) = get_data()
for epoch in range(no_epochs):
	print("Epoch:",epoch)
	for j in range(train_data.shape[0]):
		print("Training Example",j+1)
		...
		(convLayer) = network.ConvolutionalLayer(z,kernellist,biaseslist,filter_size,stride_length,no_filters,padding)
		...
		(delta_weights,deltaW,deltabias,delta_transform_matrix) = BackPropagation(network,Y[j],predicted,z,Weights,biases, kernellist, biaseslist, poolLayer, convLayer,transform_matrix,alpha,label_info[j])
		...
		(kernels,Biases,Weights,biases,transform_matrix)= gradient_descent(Weights,biases,delta_weights,kernels,Biases,deltaW,deltabias, learning_rate,transform_matrix,delta_transform_matrix)
		for i in range(kernels.shape[0]):
			kernellist[i]=kernels[i]
			biaseslist[i]=Biases[i]
		#print (kernellist,"\n\n")
		#print(biaseslist,"\n\n")
		#print(Biases,"\n\n")
		#print(Weights,"\n\n")
